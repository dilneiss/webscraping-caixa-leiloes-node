...
module.exports = class ConsultaImoveis {
    ...
    async consultar(states) {
        ...
        for (let estado of estados) {
            ...
            let carregamentoCidades = await this.selecionarEstado(estado.uf, true);
            if (!carregamentoCidades) {
                console.log(`ESTADO ${estado.uf} não tem cidades`)
                return []
            }

            estado.cidades = carregamentoCidades;

            for (let cidade of estado.cidades) {
                console.log('Buscando cidade', cidade.nome);
    ...
    async capturarImoveisCidade(uf, cidade) {
        try {
            ...
            let carregamentoBairros = await this.selecionarEstado(uf, false);
            if (!carregamentoBairros) {
                console.log(carregamentoBairros)
    ...
    async selecionarEstado(uf, getCities) {

        console.log("{ selecionarEstado }", uf)

        let result = await this.page.evaluate((uf) => {
            if ($(`#cmb_estado option[value='${uf}']`).length == 0)
                return false
            $(`#cmb_estado`).val(`${uf}`);
            console.log('selecionou');
            selecionaEstado();
            console.log('chamou metodo selecionaEstado');
            return true;
        }, uf);
        ...
        return await this.page.evaluate(() => {
            let cidades = [];
            $('#cmb_cidade option').each(function() {
                if ($(this).text() != 'Selecione'){
                    cidades.push({"nome": $(this).text()});
                }
            });
            console.log('listou cidades');
            return cidades;
        });
